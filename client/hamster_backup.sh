...
/usr/bin/rsync -aAE --delete --numeric-ids --exclude-from=/etc/hamster_backups/rsync_excl.list --hard-link -e "ssh -i /etc/hamster_backups/id_rsa_hamster" / root@hamster.stusta.mhn.de:.
EC=${?}
if [ ${EC} -gt 0 ]; then
